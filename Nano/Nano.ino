...
void GetCardDirection() {
  if (!strcmp(CardID, "24051E4C")) {
    lcd.clear();
    SmartPrint("Heading: 408", 0);
    SmartPrint("Head right to your destination", 1);
  } else if (!strcmp(CardID, "25EE6BC7")) {
    lcd.clear();
    SmartPrint("Heading: 307", 0);
    SmartPrint("Head right to elevator, down 1 floor(s)", 1);
  } else if (!strcmp(CardID, "35BE74C7")) {
    lcd.clear();
    SmartPrint("Heading: 813", 0);
    SmartPrint("Head right to elevator, up 4 floor(s)", 1);
  } else if (!strcmp(CardID, "85896CC7")) {
    lcd.clear();
    SmartPrint("Heading: 109", 0);
    SmartPrint("Head right to elevator, down 3 floor(s)", 1);
  } else if (!strcmp(CardID, "A5E872C7")) {
    lcd.clear();
...
void WaitSerial() {
  ...
  strcpy(StrBuff, "");
  int start = millis();
  int delays = random(500, 1000);
  while (millis()-start<delays) {
    if (Serial.available()) {
      int temp;
